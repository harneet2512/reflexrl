"""Kaggle T4: PPO reflex policy on DTC, 3 seeds: resumable queue with an 11 h session deadline.

Previous sessions' runs (checkpoints, resume.pt, metrics) come in through the
private dataset harneetb/reflexrl-runs-ppo when it exists; finished runs are
...
rc = sh(f"{sys.executable} -u scripts/run_queue.py experiments/configs/plan_kaggle_ppo.json --n-envs 12")
