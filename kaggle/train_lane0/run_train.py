"""Kaggle T4 training lane 0: resumable queue with an 11 h session deadline.

Previous sessions' runs (checkpoints, resume.pt, metrics) come in through the
private dataset harneetb/reflexrl-runs-lane0 when it exists; finished runs are
...
rc = sh(f"{sys.executable} -u scripts/run_queue.py experiments/configs/plan_kaggle_lane0.json --n-envs 12")
