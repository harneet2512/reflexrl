"""Kaggle T4: teacher-quality ablation (action-cloned teacher): resumable queue with an 11 h session deadline.

Previous sessions' runs (checkpoints, resume.pt, metrics) come in through the
private dataset harneetb/reflexrl-runs-ablation when it exists; finished runs are
...
rc = sh(f"{sys.executable} -u scripts/run_queue.py experiments/configs/plan_ablation.json --n-envs 12")
