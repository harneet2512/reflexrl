"""Kaggle T4: fit the perception student (Qwen+Jev labels); Jev stays the decision maker."""
import glob
import os
import shutil
import subprocess
...
sh(f"{sys.executable} -m pytest -q /kaggle/working/repo/tests")  # full suite incl. ViZDoom tests
rc = 0
for s in ("dtc",):
    rc |= sh(f"{sys.executable} -u scripts/build_perception_teacher.py --scenario {s} --labels {labels} "
