"""Kaggle T4 training lane 1: resumable queue with an 11 h session deadline.

Previous sessions' runs (checkpoints, resume.pt, metrics) come in through the
private dataset harneetb/reflexrl-runs-lane1 when it exists; finished runs are
...
rc = sh(f"{sys.executable} -u scripts/run_queue.py experiments/configs/plan_kaggle_lane1.json --n-envs 12")
