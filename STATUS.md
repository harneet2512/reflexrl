...
5. **The trained policy really is looking.** Blind it and it scores 0.50 against 7.35
   ...
   Given a frozen frame ReflexRL drops to 0.35 while PPO still manages 3.30, so ReflexRL
   is the arm that depends on live vision most. The discrimination sits in turn
   direction, not fire discipline: all arms fire ~85-90% of the time regardless, but
   ReflexRL's steering tracks the true monster side best (+0.32 against PPO's +0.21) and
   it is the only arm that reverses its turn for a monster on the right.
...
| **Blindfold + alignment check**, all 3 arms | done | blind 0.50 vs seeing 7.35; direction sensitivity ReflexRL +0.32, control +0.26, PPO +0.21 |
